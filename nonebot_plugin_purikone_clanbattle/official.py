...
async def _(event: Event, state: T_State):
    if event.get_event_name() == EventType.GROUP_AT_MESSAGE_CREATE:
        state["group_id"] = event.group_id
        state["user_id"] = event.author.id
    if event.get_event_name() == EventType.AT_MESSAGE_CREATE:
        state["group_id"] = event.guild_id + "-" + event.channel_id
        state["user_id"] = event.author.id
...
async def build_message(res: list[dict]) -> Message:
    ...
    for d in res:
        for k,v in d.items():
            if k == "text":
                m.append(MessageSegment.text(v))
            elif k == "at":
                nickname = await pcr.user.get_user_nickname(v)
                m.append(MessageSegment.text(nickname[0]["text"]))
    return m
