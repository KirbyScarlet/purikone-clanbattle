...
async def calc(nums: list[int], text: Callable, message: Message) -> Message:
    if len(nums) == 3:
        nums.sort()
        a, b, c = nums
        if a+b<c:
            message.append(text(f"不够，还差{c-a-b+1}，进一刀{int((c-a-b)*30/7)+1}吃满补"))
        elif a+b==c:
            message.append(text(f"正好收了"))
        else:
...
pair = on_command("合刀", rule=check_bot)

@pair.handle()
async def _pair(bot: Bot, event: Event, msg: Message = CommandArg()):
    error = False
    try:
    ...
    if isinstance(event, Eventv11):
        reply = MessageSegmentv11.reply(event.message_id)
        text = lambda x: MessageSegmentv11.text(x)
        message = Message()
    ...
    await pair.finish(await calc(nums, text, message))
